...
def create_model(config):
    using_latents = getattr(config, "use_latents", False) or getattr(config, "vae", False)
    model = DiTTransformer2DModel(
        sample_size=16 if using_latents else config.image_size,  # 16 for 128px images with 8x downsampling
        ...
        num_layers=24,  # DiT-S/2 uses 12 layers
        num_attention_heads=16,  # DiT-S/2 uses 6 heads
...
def create_noise_scheduler(config):
    if config.noise_scheduler == "DDPM":
        noise_scheduler = DDPMScheduler(
            ...
            clip_sample=False,  # Critical: False for latent space diffusion
