...
def main():
    ...
    checkpoint_list = ["0014", "0029", "0044", "0059", "0074"]
    ...
    for checkpoint in checkpoint_list:
        ...
        for i in range(num_timestep_groups):
            ...
            time_step_group_loss = calculate_time_step_group_loss(time_step_low_bound, time_step_high_bound, model, dataloader, noise_scheduler, device)

            print(f"Checkpoint {checkpoint} - Time step group {i} ({time_step_low_bound}-{time_step_high_bound}): {time_step_group_loss}")
    ...
    # Plot each timestep group as a separate line
    for i in range(num_timestep_groups - 1):
        ...
        plt.plot(checkpoint_numbers, timestep_group_losses[i], 
                ...
                label=f'Group {i} (t={time_step_low}-{time_step_high})')
    
    plt.xlabel('Checkpoint', fontsize=12)
    plt.ylabel('Loss', fontsize=12)
    plt.title('Timestep Group Loss vs Checkpoint', fontsize=14, fontweight='bold')
    plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    
    # Save the plot
    output_path = Path(__file__).parent.parent / "logs" / "DiT20250726_235632" / "timestep_group_loss_plot_uniform.png"
