...
def save_image_grids(
    ...
    """Save images in 8x8 grid(s). Creates multiple grids if needed.

    The grid canvas uses the mode and size of the first image.
    """
    if not images:
        return
    ...
    num_pages = (num_images + per_grid - 1) // per_grid

    for page_idx in range(num_pages):
        start = page_idx * per_grid
        end = min(start + per_grid, num_images)
        page_images = images[start:end]

        grid = Image.new(mode, (cols * img_w, rows * img_h))
        ...
        suffix = "" if num_pages == 1 else f"_{page_idx:02d}"
        grid.save(os.path.join(output_dir, f"grid{suffix}.png"))
...
def generate_images(
    ...
    if cfg_enabled:
        # Cycle labels 0..9 to match 10 classes regardless of num_images
        class_labels = torch.tensor([i % 10 for i in range(num_images)], dtype=torch.long, device=device)
        print(f"Generating {num_images} images with class labels cycling 0-9 and CFG scale {guidance_scale}")
    else:
    ...
    # Also save 8x8 grid(s)
    save_image_grids(images, output_dir, rows=8, cols=8)
    print("✅ Saved grid image(s)")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--cfg-enabled", action="store_true", help="Enable CFG with class labels cycling 0..9")
