        ...
        for epoch in range(self.config.num_epochs):
            ...
            if torch.cuda.is_available():
                ...
                if (epoch + 1) % self.config.validation_epochs == 0:
                    ...
                    val_loss = self.validation_loss(model, ema_model, validation_dataloader, self.config, epoch, global_step, EMA = False, timestep_lower_bound = self.config.training_timestep_lower_bound, timestep_upper_bound = self.config.training_timestep_upper_bound)
